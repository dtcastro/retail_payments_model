...
CASH = 0
DEBIT = 1
ratio_threshold_for_accepting = 0.25
...
class Payee(Agent):
    ...
    """
    def __init__(self, unique_id, model, accepts_pix):
...
#        print('Discounts: ' + str(self.discounts))
    ...
    def period2(self):
        ...
        self.lost_payments_ratio = self.lost_payments/(self.lost_payments + self.payments)
        if self.lost_payments_ratio > ratio_threshold_for_accepting:
            self.accepts_pix = 1
...
class Payeer(Agent):
    ...
    """
    def __init__(self, unique_id, model, uses_pix):
        super().__init__(unique_id, model)
        self.uses_pix = uses_pix
#        self.discounts = discounts
        
    def period1(self):
        print('Consumer ' + str(self.unique_id) + str(self.uses_pix) + 'step1')
        # chooses a merchant 
        ...
        if payees.favorite_instrument == self.favorite_instrument:
            #print('buy')
            merchant.sales += 1
        else:
